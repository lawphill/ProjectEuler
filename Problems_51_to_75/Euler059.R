...
#   encryption key on the cipher text, restores the plain text; for example, 65 XOR 42 = 107, then 107
#   XOR 42 = 65.
...
#   cipher.txt (right click and 'Save Link/Target As...'), a file containing the encrypted ASCII codes,
...
ascii_to_int <- function(x) { strtoi(charToRaw(x),16L) }
int_to_ascii <- function(x) { rawToChar(as.raw(x)) }
bits_to_int <- function(x) { sum(2^(which(x==1)-1)) }

space <- c(32)
lowercase_ascii <- 97:122

length_key <- 3
key <- rep(0,length_key)
for(i in 1:length_key){
  smalld <- d[seq.int(i,length(d),length_key)]
  for(j in lowercase_ascii){
    decrypted <- mapply(function(x,y) bits_to_int(xor(intToBits(x),intToBits(y))), smalld, j)
    spaces <- sum(decrypted == 32)
    if(spaces > 30){
      if(key[i] != 0){
        print("Key is already found")
        print(i)
        print(key[i])
      }else{
        key[i] <- j
...
long_key <- rep(key,length.out = length(d))

decrypted <- mapply(function(x,y) bits_to_int(xor(intToBits(x),intToBits(y))), d, long_key)
print(int_to_ascii(decrypted))
print(sum(decrypted))