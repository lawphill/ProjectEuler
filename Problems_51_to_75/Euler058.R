...
# Add this four times to get first four diagonals, then increment inc_next_diag by 2 and repeat
inc_next_diag <- 0
# 1,3,5,7,9, 13,17,21,25, 31,etc...
...
while( prime_ratio >= thresh){
  side_length <- side_length + 2
  inc_next_diag <- inc_next_diag + 2
  all_diags <- all_diags + 4
  for(i in 1:4){
    curr <- curr + inc_next_diag
    if(sum(curr %% 2:ceiling(sqrt(curr)) == 0)==0){
      prime_diags <- prime_diags + 1
    }
  }
  prime_ratio <- prime_diags / all_diags
}
print(side_length)
